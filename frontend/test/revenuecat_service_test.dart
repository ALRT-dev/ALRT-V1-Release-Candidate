...
void main() {
  ...
  test(
    ...
    () async {
      ...
      expect(
        await rc.restore(),
        isFalse,
        reason: 'restore needs a known identity',
      );
    },
  );
  ...
  group('store errors are explained, a cancel is not an error', () {
    String code(PurchasesErrorCode c) =>
    ...
    test('cancelled purchase shows no error', () {
      expect(
        purchaseErrorMessage(
          PlatformException(
            code: code(PurchasesErrorCode.purchaseCancelledError),
          ),
        ),
        isNull,
      );
    ...
    test('network, unavailable product and unknown codes are named', () {
      ...
      expect(
        purchaseErrorMessage(
          PlatformException(
            code: code(PurchasesErrorCode.productNotAvailableForPurchaseError),
          ),
        ),
