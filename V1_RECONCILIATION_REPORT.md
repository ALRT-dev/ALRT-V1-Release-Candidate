...
**Status:** Stage 6A (final alert-engine reconciliation — the six open questions from §20) complete — see §21. Stage 7A (Admin Portal audit and V1 scope — read-only, no application code changed) complete — see §22. Stage 7B (Admin backend hardening — the backend prerequisites from §22.4: moderation endpoint, role enforcement, audit log, pagination/validator fixes) complete — see §23. Stage 7C (ALRT V1 Admin Portal — a new `admin/` frontend against the existing Admin API) complete — see §24. Stage 8 (complete V1 release-readiness audit — repository-wide, thirteen parallel investigations, four small confirmed-safe fixes applied) complete — see §25. **RELEASE STATUS (as of Stage 8): READY WITH CONDITIONS — see §25.20.** Stage 9A (Journey Sharing recipient experience — the P1 gap from §25.18.1: a new authorized single-journey retrieval endpoint, a recipient viewer screen and map reusing the existing Google Maps implementation, a direct notification deep link, and live updates on the existing socket/poll mechanism) complete — see §26. Stage 9B (Authentication completion — the two remaining §25.18 P1 items: a full password-reset flow built on existing SMTP/bcrypt/JWT infrastructure, and a from-scratch Microsoft OAuth investigation that found and fixed two real frontend defects but left the sign-in button intentionally hidden pending real Azure credentials and a product decision on account linking) complete — see §27. Stage 10 (final V1 production-readiness audit — seven parallel investigations, a fresh cross-repo historical-reconciliation pass, and an independent final security sweep; found one new P0 privacy defect and one new safety-relevant push-notification bug, neither fixed per this audit-only stage's own instruction) complete — see §28. **RELEASE STATUS (as of Stage 10): NOT READY — see §28.15.** Stage 11 (release blocker fixes — the three genuine blockers §28.15 identified: the public-endpoint GPS privacy leak fixed with the existing `withPublicCoords()` mechanism, the `familyHazardProximity` double-JSON-encoding bug fixed at its root cause, and a real (not fake) production Android signing/CI structure added without inventing credentials) complete — see §29. **RELEASE STATUS (as of Stage 11): NOT READY FOR PRODUCTION, READY FOR INTERNAL TESTING ONCE CONFIGURED — see §29.5.** Stage 12 (production configuration and internal release preparation — a consolidated, classified configuration checklist reconciling §28+§29, precise external-service/GitHub-secret requirements for Android, Google/Firebase, Microsoft OAuth, RevenueCat, email, the Admin Portal, and backend infrastructure, the internal release sequence, and the full real-device QA matrix; documentation only, no application code changed) complete — see §30. **RELEASE STATUS (as of Stage 12): unchanged — NOT READY FOR PRODUCTION, READY FOR INTERNAL TESTING ONCE THE §30.1 CHECKLIST IS COMPLETE — see §30.1/§30.11.** Stage 13 (Firebase production configuration — fixed `firebase_options.dart`'s dev/prod asymmetry on Android with a real, already-registered app identity; iOS left unchanged because no second iOS app is registered in Firebase yet, documented as a remaining external-configuration action, not invented) complete — see §31. **RELEASE STATUS (as of Stage 13): unchanged — NOT READY FOR PRODUCTION, READY FOR INTERNAL TESTING ONCE THE §30.1 CHECKLIST (now including §31.6's real iOS Firebase app registration) IS COMPLETE.** Stage A (isolated internal-test environment, test infrastructure only — a separate test-backend configuration, a separate Admin Portal test build, and a new, isolated `android-test.yml` CI workflow, all pointed at a proposed but not-yet-deployed test backend; no production file modified) complete — see §32. A subsequent local verification pass (§33) proved the Stage A code/config end to end against a real, session-local Postgres+PostGIS database and backend process (register, login, family, hazard, admin-portal visibility, cross-database isolation all HTTP-verified), found and documented a genuine pre-existing migration-history ordering defect (not fixed, per explicit instruction — see §33.1), and confirmed no real external deployment exists yet — no cloud hosting, DNS, GitHub Environment/secret, or real Firebase/RevenueCat credentials were available in this session to create one. This is infrastructure preparation and local verification, not a release-readiness change — the production release status above is unaffected. Application code has been changed in this repository only, across stages up to and including Stage 13 — see §17, §18, §19, §20, §21, §23, §24, §25, §26, §27, §29, and §31; Stage 10 (§28), Stage 12 (§30), and Stage A (§32–§33, test infrastructure/config and local verification only) are audit/documentation/config-only and changed no production application code. §34 (Section B of the approved TEST environment plan) adds repository-side-only preparation — a documented, verified Prisma TEST-database baseline script, a corrected first-boot sequence for `docker-compose.test.yml`, and additive iOS Dev TestFlight CI/Fastlane scaffolding targeting `com.safetyalrt.alrt.dev` — with nothing deployed and no production file touched. §35 fixes one real bug that audit found: `share_alert.dart` built its public share link from the hardcoded `kUrlBase` production constant rather than the flavor-gated `providerOfBaseUrl`, so a DEV/TEST build's "Share" action generated a broken production link instead of one pointing at whatever backend the build actually talks to — fixed by routing it through the existing provider, no new mechanism introduced. No original repo (`frontendV2`, `backendV2`, `askalrt`, `V2-Claude`, `v3`) has been modified, no branches were merged wholesale, and nothing has been deployed.
...
## 31. Firebase Production Configuration (Stage 13)

Stage 12 is accepted. This stage resolves the one code-adjacent gap §30.3/§30.11 flagged but explicitly left untouched: `firebase_options.dart`'s dev/prod asymmetry.

### 31.1 Trace — what was actually wrong, and what wasn't

**Traced:** `firebase_options.dart`, `android/app/src/dev/google-services.json`, `android/app/src/prod/google-services.json`, `android/app/build.gradle.kts`, `ios/Runner/GoogleService-Info.plist`, `ios/Runner.xcodeproj/project.pbxproj` (schemes, build configurations, resource references), `lib/others/app_bootstrap.dart`, `lib/main_dev.dart`/`lib/main_prod.dart`, and the FlutterFire-documented pattern for multi-flavor Firebase initialization (verified against current community/FlutterFire guidance via web search, not assumed — see sources below).

**What was already correct, confirmed by direct comparison, not previously re-verified this precisely:**
- **Android's native config was never the problem.** `google-services.json` is genuinely different per flavor source-set (`src/dev/`, `src/prod/`) but the two files are byte-identical (`diff` confirms zero difference) — because the Google Services format registers *all* of a project's apps inside every copy of the file, and the Google Services Gradle plugin (`com.google.gms.google-services`, applied in `app/build.gradle.kts`) automatically selects the `client` entry matching the build's actual `applicationId` at build time. Both `com.safetyalrt.alrt` and `com.safetyalrt.alrt.dev` are real, separately-registered Firebase apps within the one `alrt-a6539` project, each with its own real `appId`; no root-level `app/google-services.json` exists to override this. **Native Android Firebase registration was already correct for both flavors before this stage.**
- **iOS has only one registered app.** `ios/Runner/GoogleService-Info.plist` — one file, one `PBXFileReference`/`PBXBuildFile` in `project.pbxproj`, shared by both the `dev` and `prod` Xcode schemes (confirmed: `dev.xcscheme` uses `Debug-dev`/`Release-dev` build configurations, `prod.xcscheme` uses `Debug-prod`/`Release-prod`/`Profile-prod` — real, separate configurations exist, but no per-configuration Copy Bundle Resources step or Run Script ever swaps in a different plist). Its `BUNDLE_ID` is `com.safetyalrt.alrt` (prod) and its `GOOGLE_APP_ID` is `1:705221000399:ios:42910fba9ef33a48858bba` — there is genuinely no second Firebase iOS app registered anywhere for `com.safetyalrt.alrt.dev`. This is **not** a code bug; there is no config file to select between because only one exists.

**What was actually wrong:** `lib/firebase_options.dart` hardcodes exactly one `FirebaseOptions` value per platform (the prod Android `appId`, the sole/prod iOS `appId`), and `lib/others/app_bootstrap.dart`'s `_initializeFirebase()` called `Firebase.initializeApp(options: DefaultFirebaseOptions.currentPlatform)` unconditionally — the same call, same values, regardless of `AppBootstrap`'s own `flavor` parameter (`AppFlavor.dev`/`AppFlavor.prod`), even though the same class already conditionally overrides `providerOfAppInfo`/`providerOfBaseUrl` by flavor two lines below it. This was the only `Firebase.initializeApp` call site in the entire `lib/` tree (confirmed by grep — no background-isolate FCM handler exists to duplicate the problem elsewhere). The explicit `options:` argument is not cosmetic: per FlutterFire's own documented multi-environment pattern (separate `firebase_options_<flavor>.dart` files selected by a flavor-aware entry point — see sources), passing explicit `FirebaseOptions` to `Firebase.initializeApp()` is exactly the mechanism that determines which Firebase app identity a Flutter app actually uses at runtime, independent of whatever the native platform layer would otherwise auto-select. So on Android specifically, a `.dev`-flavor build's Dart code was overriding the correct, per-flavor native registration with the prod app's identity — the opposite of the native config's own correct behavior.

**Answering the four required questions directly:**
- **What is currently development-only?** Nothing in `firebase_options.dart` itself was development-only — the bug was that dev and prod shared the identical (prod) values. `google-services.json`'s two source-set copies are genuinely per-flavor at the native layer.
- **What does production configuration require?** No change — `firebase_options.dart` (prod values) was already correct and remains completely untouched by this stage's fix.
- **Can the app safely select the correct Firebase project for production?** Yes, and this was already true before this stage — `AppFlavor.prod` always resolved (and still resolves) to `DefaultFirebaseOptions.currentPlatform`, the FlutterFire-CLI-generated, unmodified file.
- **Do Android and iOS behave consistently?** No, and this stage does not make them fully consistent — it makes Android *correct* (a real fix) while leaving iOS *unchanged* (still using the prod app identity on `.dev` builds), because a correct iOS fix requires a Firebase app registration that does not exist in this environment and cannot be created without console access (§31.6).
- **Could debug/dev configuration accidentally ship in a production build?** No — before or after this fix. `AppFlavor.prod` never referenced dev values at any point; the bug ran in the opposite direction (a dev build silently used the prod identity), which is a correctness/telemetry-attribution problem for the dev build, not a risk of dev config leaking into production.

Sources consulted for the FlutterFire multi-flavor pattern (web search, not application-specific — used to confirm the mechanism, not copied from): [How to Setup Flutter & Firebase with Multiple Flavors using the FlutterFire CLI](https://codewithandrea.com/articles/flutter-firebase-multiple-flavors-flutterfire-cli/), [Flavoring for Multiple Environments in a Flutter App](https://kashifchandio.medium.com/flavoring-for-multiple-environments-in-a-flutter-app-use-different-firebase-projects-for-dev-66dcca61bf89), [How to Set Up Flavor-Based Firebase in Flutter Projects](https://medium.com/@melik.bozkurt/how-to-set-up-flavor-based-firebase-for-different-environments-in-flutter-projects-14f25870da20).

### 31.2 Fix

**Minimum safe change — two files, additive, no unrelated behaviour touched:**

1. **New file `frontend/lib/firebase_options_dev.dart`** — a hand-authored (not FlutterFire-CLI-generated) `DefaultFirebaseOptionsDev` class, structurally identical to the existing `DefaultFirebaseOptions`. Its Android block uses the real `com.safetyalrt.alrt.dev` `appId` (`1:705221000399:android:e626de52935e48e0858bba`) copied verbatim from the already-committed `android/app/src/dev/google-services.json` — not invented, independently confirmed by parsing that file's own JSON this stage. `apiKey`/`messagingSenderId`/`projectId`/`storageBucket` are project-scoped (identical across every app in `alrt-a6539`, confirmed by comparing both `google-services.json` copies), so they match `firebase_options.dart`'s Android values exactly; only `appId` differs. Its iOS block is **deliberately identical** to `firebase_options.dart`'s existing iOS values — see §31.6.
2. **`frontend/lib/others/app_bootstrap.dart`** — `_initializeFirebase()` now selects `DefaultFirebaseOptionsDev.currentPlatform` when `flavor == AppFlavor.dev`, and `DefaultFirebaseOptions.currentPlatform` (unchanged) otherwise. One `import` line added, one `options:` value changed from a bare reference to a ternary. Nothing else in the method changed — `FirebaseSessionService.initializeAppCheck()`'s call order and its own `kDebugMode`-based provider selection (a separate, already-correct axis, debug/release build mode rather than dev/prod flavor) were not touched.

**Why application code, not generated config, per the task's own stated preference:** the native, generated config (`google-services.json`) was never the problem — it already correctly differentiates per flavor via the Google Services Gradle plugin, confirmed in §31.1. The bug was specifically that Dart-side code passed the same explicit `FirebaseOptions` regardless of flavor, overriding what the native layer would otherwise select. There is no generated-config-only fix available for an application-logic override; the flavor-conditional selection had to happen where the override itself happens.

**Nothing embedded that qualifies as a secret:** Firebase client `apiKey`/`appId` values are not access-granting secrets — access is controlled by Firebase Security Rules and App Check, not by keeping these identifiers private (the same finding already independently reached in §28.10 P3 about the values already committed in `firebase_options.dart` and both `google-services.json` files; this stage's new file uses the identical class of already-public, already-committed identifiers, copied from a file already in this repository, not a new exposure).

### 31.3 Development behaviour (after this fix)

`AppFlavor.dev` builds (both `flutter run --flavor dev` and the `android-apk.yml` CI pipeline, which builds `-t lib/main_dev.dart`) now initialize Firebase with `com.safetyalrt.alrt.dev`'s real, separately-registered `appId` on Android — matching what the native `google-services.json` already correctly exposes for that flavor, for the first time making the Dart-side and native-side identities agree. On iOS, dev-flavor behaviour is **unchanged** — it continues to initialize with the prod iOS app identity, exactly as it did before this stage, because no second iOS app exists to select instead (§31.6). This is a real, standing limitation, not silently hidden: it is called out in the new file's own header comment and here.

### 31.4 Production behaviour (after this fix)

**Completely unchanged.** `AppFlavor.prod` still resolves to `DefaultFirebaseOptions.currentPlatform` — the original, FlutterFire-CLI-generated `firebase_options.dart`, not modified by this stage in any way (`git diff` confirms zero changes to that file). The production build selection path is deterministic: `main_prod.dart` → `AppBootstrap(flavor: AppFlavor.prod)` → `_initializeFirebase()`'s ternary evaluates the `else` branch → the exact same `DefaultFirebaseOptions.currentPlatform` call that ran before this stage. There is no code path by which a production build can select `DefaultFirebaseOptionsDev` — the ternary is keyed on the same `AppFlavor` enum value that already gates every other flavor-conditional behaviour in this file (`providerOfAppInfo`/`providerOfBaseUrl`), and that value is fixed at compile/entry-point time by which of `main_dev.dart`/`main_prod.dart` was used to build the binary, not by any runtime-mutable state.

### 31.5 Build selection mechanism

Confirmed by direct trace of the existing, unmodified build wiring — nothing about *how* a flavor is selected was changed this stage:
- **Flutter/Dart layer:** `lib/main_dev.dart` calls `AppBootstrap(flavor: AppFlavor.dev)`; `lib/main_prod.dart` calls `AppBootstrap(flavor: AppFlavor.prod)`. CI already builds with explicit `-t lib/main_dev.dart` / `-t lib/main_prod.dart` (`android-apk.yml`, `android-release.yml`, `ios-testflight.yml`) — the entry-point file, not an environment variable or runtime flag, is what fixes the flavor for a given binary.
- **Android native layer:** `android/app/build.gradle.kts`'s `productFlavors { dev { ... }; prod { ... } }` block, combined with Gradle's standard `src/<flavor>/` source-set convention, is what makes `com.google.gms.google-services` pick up `src/dev/google-services.json` vs `src/prod/google-services.json` at build time — this was already correct and untouched.
- **iOS native layer:** `dev.xcscheme`/`prod.xcscheme` select `Debug-dev`/`Release-dev` vs `Debug-prod`/`Release-prod`/`Profile-prod` build configurations, which in turn set `PRODUCT_BUNDLE_IDENTIFIER` to `com.safetyalrt.alrt.dev` vs `com.safetyalrt.alrt` — this determines the app's *bundle ID*, but does **not** currently select between two different `GoogleService-Info.plist` files, because only one exists (§31.6).

### 31.6 Remaining external Firebase configuration (STOP — cannot be completed in this environment)

**This is the one piece of this stage's task that genuinely cannot be finished here, per the explicit instruction to document and stop rather than invent it.** A correct iOS dev/prod split requires:
1. **A real Firebase Console action**: register a second iOS app in the `alrt-a6539` project for bundle ID `com.safetyalrt.alrt.dev` (Firebase Console → Project settings → Add app → iOS, or via the `flutterfire configure` CLI against a real, authenticated Firebase project — neither of which this environment has credentials or network access for).
2. **Download the resulting `GoogleService-Info-dev.plist`** (or equivalent name) that registration produces — its real `appId`/`apiKey` cannot be predicted or invented; they are assigned by Firebase at registration time.
3. **One of two follow-up code changes**, once step 2's real values exist:
   - Replace `firebase_options_dev.dart`'s `ios` block with the real values (the Dart-side fix, mirroring what this stage already did for Android) — the simpler option, and the one consistent with this stage's approach; **or**
   - Add a per-scheme Xcode Run Script build phase that copies the correct `GoogleService-Info*.plist` variant into `Runner/GoogleService-Info.plist` before the Resources copy step (the native-config-only alternative, more moving parts, not chosen here since it wouldn't reduce the actual work needed — a real dev iOS app registration is required either way).

**Until that Firebase Console action happens, iOS `.dev` builds will continue to report crash/analytics/FCM registration data under the prod iOS app's identity** — the same, pre-existing, already-documented limitation from §28.3/§30.3, unchanged by this stage. This does not block internal Android testing or iOS TestFlight (TestFlight only ever builds the `prod` flavor per `ios-testflight.yml`, which is unaffected either way), and it is not a release blocker for the same reason Stage 10 did not treat it as one — but it should be resolved before a `.dev`-flavor iOS build's push/App Check/analytics behaviour is trusted for QA purposes.

### 31.7 Verification

**CODE VERIFIED (this stage, by direct inspection — not a guess):**
- `firebase_options_dev.dart`'s Android `appId` matches, character-for-character, the `com.safetyalrt.alrt.dev` client entry's `mobilesdk_app_id` in the committed `android/app/src/dev/google-services.json`, confirmed by parsing both files with `python3 -c "import json; ..."` this stage (not eyeballed).
- `firebase_options_dev.dart`'s Android `apiKey`/`messagingSenderId`/`projectId`/`storageBucket` match `firebase_options.dart`'s existing Android values, confirmed by direct comparison — both are the same project-scoped values shared across every app in `alrt-a6539`.
- `firebase_options_dev.dart`'s iOS block matches `firebase_options.dart`'s existing iOS block exactly, by design (§31.6) — confirmed by direct comparison, not accidental.
- `app_bootstrap.dart`'s new ternary is syntactically valid Dart (manually reviewed character-by-character; no Dart analyzer is available in this environment — see below) and preserves every other line of `_initializeFirebase()`, `_onInit()`, and every other method in the file unchanged (confirmed via `git diff`, 7 insertions/1 deletion, no other hunks).
- Exactly one `Firebase.initializeApp` call site exists in `lib/` (confirmed by grep across the whole tree) — no background-isolate FCM handler or second entry point needed the same fix.
- No root-level `app/google-services.json` exists to override the two flavor-specific copies (confirmed by `find`); the Google Services Gradle plugin is applied in `app/build.gradle.kts` (confirmed by grep) — Android's native per-flavor selection was independently re-confirmed correct, not merely assumed from §28.3's prior finding.
- `git diff --stat` confirms the change is scoped to exactly two frontend files (one new, one modified) — no backend, `admin/`, or `askalrt/` file was touched, so those regression suites (Stage 6/6A/7B/8/9A/9B/Ask ALRT/Admin Portal) are unaffected by this stage and were not re-run; re-running them would prove nothing new since this stage's diff cannot reach any code path they exercise.

**NOT verified, and not claimed, this stage — REAL DEVICE VERIFIED: NO, BUILD VERIFIED: NO:**
- `flutter analyze`/`flutter build`/`dart analyze` were never run — no Flutter/Dart/Android/iOS tooling exists in this environment (`which flutter dart adb xcodebuild pod` all fail, unchanged since every prior stage). The Dart syntax correctness above is a manual review, not a compiler-verified fact.
- Whether a `.dev`-flavor build actually receives FCM pushes, registers correctly with Crashlytics/Analytics (if used), or passes App Check under the corrected Android app identity has not been confirmed on a real device or emulator — this requires the real-device QA pass already tracked as open in §30.10, not a new requirement this stage invents.
- iOS dev-flavor behaviour was intentionally left unchanged (§31.6), so there is nothing new to device-verify for iOS from this stage's change specifically.

### 31.8 Stop condition honored

No deployment was performed. No production billing was touched — `BILLING_ENABLED` remains untouched, not referenced by this stage's change. Microsoft OAuth was not touched. No new feature work was done — this is a bug fix scoped to exactly the dev/prod Firebase-identity mismatch named in the task, nothing broader. The one piece of this task that requires real external Firebase configuration not available in this environment (a second iOS app registration) was identified, documented in full in §31.6, and explicitly not invented or worked around with a placeholder value — per the stop condition, this is reported as remaining work, not silently completed. No secret was committed: every value in `firebase_options_dev.dart` is a client-side Firebase app identifier already established as non-sensitive (§28.10 P3), copied from a file already committed to this repository, not a new credential.


## 32. Isolated Internal-Test Environment — Stage A: Test Infrastructure

Preceded by an extended multi-turn audit (Google Maps key reconciliation against live Cloud Console evidence, then a read-only design pass for a fully isolated internal-test environment) — not written up as its own numbered section since no code changed during those turns; the architecture that pass produced is what Stage A implements. This stage builds only the test-infrastructure code/configuration named in the approved plan: a separate test backend's configuration, a separate Admin Portal test build, and a new, isolated Android test CI workflow. **No production file was modified.** Full detail: `backend/docs/TEST_ENVIRONMENT.md`.

### 32.1 What this stage found before building anything

Two facts discovered during the read-only design pass materially shaped Stage A:
1. **The only existing "test" pipeline was never isolated.** `android-apk.yml`'s dev-flavor build sets `DEV_BASE_URL=https://api.safetyalrt.com` — it has always pointed at the real production backend and database. This stage does not change that file (per the stop condition) but it is why a genuinely separate test backend is necessary, not optional.
2. **Production's backend URL cannot be redirected by any configuration.** `kUrlBase` (`frontend/lib/api/endpoints.dart`) is a hardcoded Dart constant with no environment override — only the dev-flavor URL (`providerOfBaseUrlDev`, via `DEV_BASE_URL`) is configurable. This is the load-bearing fact that makes the whole isolation design safe: no test configuration can ever redirect a real production build's traffic, only the dev-flavor build this stage's new workflow already targets.

### 32.2 Files changed

| File | Change |
|---|---|
| `backend/.env.test.example` | New. Full env-var template for the test backend, mirroring `.env.default`'s structure, `NODE_ENV=test` and `BILLING_ENABLED=false` set explicitly, every isolation-relevant var carrying an explanatory comment. |
| `backend/docker-compose.test.yml` | New. A local reference deployment: its own Postgres+PostGIS container (`postgres-test`, port 5433, volume `pgdata_test`) — distinct name/port/volume from the existing `dev`/`prod` compose files, so it can run alongside them without collision and is a separate database by construction. |
| `backend/.gitignore` | Added `.env.test` (a real one, once created locally, must never be committed — same protection already given to `.env.dev`/`.env.prod`). |
| `backend/docs/TEST_ENVIRONMENT.md` | New. Architecture diagram, exact deploy commands, the isolation-verification table, and the external-actions checklist — see §32.4 below for the highlights. |
| `admin/.env.test` | New, and deliberately **committed** (not gitignored) — points the Admin Portal's test build at the proposed `https://api-test.safetyalrt.com`. A base URL is not a secret (the production one is already hardcoded in plaintext elsewhere in this repo); committing it is what makes the test build reproducible in CI without a new secret. |
| `admin/.gitignore` | Added a `!.env.test` exception alongside the existing `!.env.example` one, with a comment explaining why. |
| `admin/package.json` | Added `"build:test": "tsc -b && vite build --mode test"`. |
| `frontend/.github/workflows/android-test.yml` | New. Manually-triggered (`workflow_dispatch` only) CI workflow: identical to `android-apk.yml`'s dev-flavor build except `DEV_BASE_URL` is sourced from a `test`-Environment-scoped secret (`TEST_BACKEND_URL`) instead of the hardcoded production URL, and it refuses to run at all if that secret is unset. Publishes only as a workflow artifact (`ALRT-android-test-apk`) — never touches the `dev-latest` GitHub Release `android-apk.yml` owns. |

**Confirmed untouched, by `git diff --stat` against every named file in the stop condition:** `android-release.yml`, `ios-testflight.yml`, every backend `src/` file, every frontend `lib/` file, `build.gradle.kts` — all show zero diff.

### 32.3 Test backend architecture and proposed URL

A second deployment of the exact same backend Docker image (no code fork), with its own Postgres+PostGIS instance, reachable at the **proposed** (not yet real) domain `https://api-test.safetyalrt.com` — matching this repo's existing `api.safetyalrt.com`/`admin.safetyalrt.com` naming convention. `NODE_ENV=test` (not `prod`) — a deliberate choice, not an oversight: `backend/src/index.ts` only runs the in-process scheduled jobs (hazard ingestion, expiry sweeps, check-ins) when `NODE_ENV=prod`, hardcoded (`runScheduledTasksInDev = false`), so the test backend will not auto-ingest real-world hazards. Documented workaround: the existing Admin Portal "Sync" action (`POST /api/admin/hazards/sync-external`, already built) triggers ingestion manually regardless of `NODE_ENV`. Loosening that gate was considered and explicitly not done — it would mean editing shared scheduling code in `index.ts`, out of Stage A's "test infrastructure only" scope.

### 32.4 Isolation verification

| Property | Method | Result |
|---|---|---|
| Test backend → test database | Code inspection: `DATABASE_URL` is the sole DB connection var, sourced only from whichever `.env.${NODE_ENV}` file is loaded (`.env.test` here) — no fallback/second connection string exists anywhere in the codebase. | CODE VERIFIED |
| Test Admin Portal → test backend | **Empirically run this stage**: `npm run build:test` was executed and its output bundle `grep`-confirmed to contain `api-test.safetyalrt.com`; the regular `npm run build` was then re-run and confirmed to contain **no** trace of that string. Both builds also re-ran the full Vitest suite in between (21/21 passing, unaffected by the new `.env.test` file). | BUILD VERIFIED |
| Android test build → test backend | Code/YAML inspection: `android-test.yml`'s only substantive difference from `android-apk.yml` is `DEV_BASE_URL`'s source; `providerOfBaseUrlDev` (`base_url_provider.dart`) has exactly one override path, `DEV_BASE_URL`, already traced in an earlier stage. YAML syntax validated with `python3 -c "import yaml; ..."`. | CODE VERIFIED — not run (no Flutter/Android tooling in this environment, same limitation as every prior stage) |
| No test component → production database | Confirmed no file this stage touches contains any reference to the production `DATABASE_URL`, production hostnames, or any code path connecting to more than one database at once. | CODE VERIFIED |
| No test notification can target production users through the test database | `notification.service.ts`'s send path resolves `UserDevice` rows entirely within whatever database the running process is connected to — no cross-database/cross-environment lookup exists anywhere in that code. A test backend with its own database structurally cannot address a production device token. | CODE VERIFIED (architectural guarantee — not exercised end-to-end; no real push was sent) |

### 32.5 New secrets required (GitHub)

Exactly one: **`TEST_BACKEND_URL`**, scoped to a new GitHub Environment named `test` (Settings → Environments → New environment). Not created by this stage — no GitHub Environment/secret-management tool is available in this session; this is a required, undone **USER ACTION**. `android-test.yml` reuses the existing repo-level secrets `GOOGLE_MAPS_API_KEY`/`googlemaps`, `GOOGLE_OAUTH_SERVER_CLIENT_ID`, `REVENUECAT_API_KEY_APPLE`, `REVENUECAT_API_KEY_GOOGLE` — none of these are new, none were created or modified, all are read-only references to what `android-apk.yml` already uses.

### 32.6 External services / actions required (none performed by this stage)

1. Provision a real Postgres+PostGIS instance for the test backend (any provider) — never the production instance or a schema within it.
2. Deploy the backend Docker image somewhere reachable with `.env.test` as its environment (exact commands in `TEST_ENVIRONMENT.md`).
3. Register/point a domain at it (`api-test.safetyalrt.com` proposed).
4. Create the GitHub `test` Environment and add `TEST_BACKEND_URL` to it.
5. Run `create-super-admin.ts` against `.env.test` once the backend is live.
6. Deploy the Admin Portal `build:test` output to a static host, and add its domain to the test backend's `CORS_ALLOWED_ORIGINS`.
7. Run `android-test.yml` manually once 1–4 are done.

### 32.7 What cannot be verified in this environment

No externally-reachable test backend exists as a result of this stage — a locally-runnable `docker-compose.test.yml` is explicitly not being represented as "a deployed test environment" (per this stage's own instruction not to conflate the two). No real device has installed the test APK; no real push notification was sent; DNS for the proposed test domain does not exist. Everything above is CODE VERIFIED or, for the Admin Portal split specifically, BUILD VERIFIED — nothing in this stage is REAL DEVICE VERIFIED or DEPLOYED-INFRASTRUCTURE VERIFIED, and none of it is claimed to be.

### 32.8 Stop condition honored

No production file was modified — verified by `git diff --stat` against `android-release.yml`, `ios-testflight.yml`, every backend `src/` file, every frontend `lib/` file, and `build.gradle.kts`, all showing zero diff. No production database, Firebase, or RevenueCat configuration was touched. The existing `ALRT App` Google Maps key was not modified, restricted, or migrated — `android-test.yml` only *reads* the existing repo-level secret, exactly as `android-apk.yml` already does. No existing production GitHub secret was created, modified, or deleted — the one new secret this stage requires (`TEST_BACKEND_URL`) does not exist yet and was not created by this session (no tool access to GitHub Environments). Real RevenueCat purchase-flow testing was not implemented — `ALRT_PLUS_TEST_UNLOCK=true` continues to be the mechanism `android-test.yml` uses, unchanged from `android-apk.yml`. No iOS test workflow was created — the dev iOS Firebase app registration this depends on (§31.6) is still outstanding. `android-apk.yml` itself was not modified — `android-test.yml` is a wholly new, separate file. `admin/.env.test`, the only committed value with a real-looking URL in it, is a hostname, not a secret, and points at infrastructure that does not exist yet — no external service was contacted, configured, or assumed reachable.


## 33. Isolated Internal-Test Environment — Local Verification Pass, and a Newly-Found Migration-Ordering Defect

Follows §32 (Stage A: test infrastructure code/config). This session attempted to genuinely deploy the isolated test environment described there. **It could not be deployed to anything externally reachable — this environment (a sandboxed code-execution container) has no cloud hosting credentials, no DNS access, and no real Firebase/RevenueCat credentials, and no tool to create a GitHub Environment or secret.** What follows is a local proof, run entirely inside this session, using the real Stage A code and configuration, against a real (but not externally reachable) Postgres+PostGIS database and a real (but not externally reachable) backend process. It is explicitly not "deployed" in the sense §32/this stage's own instruction means — nothing here persisted past this session or was ever reachable from outside it.

### 33.1 A genuine, pre-existing bug found while provisioning: migration-history ordering

Running the real `npx prisma migrate deploy` (not the `db push` shortcut every prior stage's throwaway verification has used) against a brand-new database failed immediately:

```
Applying migration `20250317000000_add_category_images`
Error: P3018 ... ERROR: relation "HazardCategory" does not exist
```

`prisma/migrations/20250317000000_add_category_images` is named with a March 2025 timestamp, but the actual `init` migration that creates `HazardCategory` (and every other table) is `20251004073622_init` — dated October 2025. Prisma applies migrations in filename-timestamp order, so it tries to run the March-dated migration first and fails because the table it modifies doesn't exist yet.

**This is not specific to this test environment.** It is a defect in the migration history itself, and it would happen to *any* fresh database provisioned via the standard `prisma migrate deploy` command — including production, if it were ever restored from scratch (disaster recovery, a new region, etc.). Production's existing database presumably never hit this because it was never provisioned via a clean `migrate deploy` from an empty database — its schema evolved incrementally over time, and by the time this migration was added the table already existed from whatever earlier process built the database.

**Not fixed this session, per explicit instruction:** the user directed that no migration file be modified, renamed, deleted, squashed, or reordered. **This is now documented as an outstanding issue requiring review before any fresh database (test or production-recovery) can legitimately use `prisma migrate deploy`.** The likely fix is a small, targeted rename of the misdated migration file(s) so they sort after `20251004073622_init` — but that decision, and its execution, is explicitly deferred to a future stage with the user's direct sign-off, not decided unilaterally here.

**Workaround used for this session's local proof only:** `npx prisma db push --skip-generate` (schema-sync, bypasses migration history entirely) — the same mechanism every prior stage's throwaway verification database has used. This is clearly a temporary local-provisioning workaround, not a substitute for fixing the underlying migration-history defect, and not something a real deployment should rely on.

### 33.2 Local isolation proof (real HTTP, real DB, run this session)

Two separate local Postgres databases were created to make isolation empirically checkable, not just asserted: `alrt_test_env` (the isolated test database, schema pushed via the workaround above, seeded via the real server-boot seeding path using `.env.test`'s `SUPER_ADMIN_*` values) and `alrt_prod_simulated` (a second, untouched database standing in for "production," carrying its own marker row and no ALRT schema at all).

| Step | Result |
|---|---|
| Register a test user | `POST /api/auth/email-password/register` succeeded, real JWT issued |
| Confirm the user exists only in the test DB | `SELECT ... FROM "User"` against `alrt_test_env` found it; the same query shape against `alrt_prod_simulated` found no `User` table at all — that database was never touched by any ALRT schema operation |
| Login | Real JWT confirmed via `POST /api/auth/email-password/login` |
| Create/read a test family | `POST /api/family/circle` + `GET /api/family/circle` succeeded, real `FamilyCircle` row created |
| Create a test community hazard | `POST /api/hazards` (multipart, matching the real upload contract) succeeded — pending moderation, as expected with no real AI provider credentials |
| Test Admin Portal sees it | The seeded test super-admin logged in via `POST /api/admin/auth/login` and `GET /api/admin/hazards?reviewStatus=pending` returned the exact hazard just created |
| Test Admin Portal build → test API only | `npm run build:test` (Vite `--mode test`) was rebuilt this session pointed at the locally-running backend and empirically confirmed (via `grep` on the compiled bundle) to contain that URL and **not** the proposed production-style test domain or any production hostname; the regular `npm run build` was separately confirmed to contain neither |
| Production database untouched | `alrt_prod_simulated`'s marker row was re-checked after every step above — unchanged throughout |
| No test notification can target production users | Architectural, re-confirmed by reading the send path again this session: `notification.service.ts` resolves `UserDevice` rows entirely within whichever database the running process is connected to (`alrt_test_env` only, this session) — no cross-database code path exists anywhere in this codebase |

### 33.3 Regression suites run against the isolated test backend

| Suite | Result | Note |
|---|---|---|
| Stage 8 (`verify_stage8_release_audit.ts`) | **7/7 passing** | |
| Stage 9A (`verify_stage9a_journey_recipient.ts`) | **13/13 passing** | |
| Stage 9B (`verify_stage9b_auth_completion.ts`) | **17/17 passing** | real local SMTP stub stood up for this run, same pattern as prior stages |
| Stage 7B (`verify_stage7b_admin_hardening.ts`) | **Did not run cleanly** | The script's hardcoded super-admin login target (`superadmin@test.local`) doesn't match this environment's deliberately distinct `SUPER_ADMIN_EMAIL` (`test-admin+isolation-check@safetyalrt.com`, chosen per `.env.test.example`'s own guidance to use an obviously-test-only, non-reused email) — a credential-naming mismatch specific to this run, not a functional regression. The capability the script would have exercised (admin login + role-gated data access) was independently confirmed working in this exact environment via §33.2's manual admin-login and moderation-queue steps. |

`npx tsc --noEmit`: clean (no backend code was changed this session).

### 33.4 GitHub Environment / secret — blocked, exact user action required

No tool available in this session can create a GitHub Environment or a secret on one (confirmed by searching this session's available GitHub tools — repository, PR, issue, and Actions-run tools exist; environment/secret management does not). `android-test.yml` was **not** run this session — it would fail immediately at its own first step (the explicit `TEST_BACKEND_URL` guard added in §32) since neither the `test` Environment nor that secret exist yet. Running it now would prove nothing beyond that the guard itself works, which is not the outcome asked for.

**Exact action required:** GitHub → this repository → Settings → Environments → New environment → name it exactly `test` → add secret `TEST_BACKEND_URL` with the real test backend's URL once one exists and is reachable.

### 33.5 What this session did NOT achieve, stated plainly

- No externally-reachable test backend exists. `alrt_test_env` and the local server process were both destroyed at the end of this session, as neither was ever reachable from outside it.
- No real domain (`api-test.safetyalrt.com` or otherwise) was registered or configured.
- No real Firebase service-account credentials were used — a fake, syntactically-valid key (matching every prior stage's established pattern) let the server boot; real push notifications cannot be sent with it.
- No real Admin Portal deployment exists — the test build was served locally within this session only.
- The GitHub `test` Environment and its secret do not exist.
- `android-test.yml` has never been run.

Everything in §33.2/§33.3 is **CODE VERIFIED** and **LOCALLY HTTP-VERIFIED (this session only)** — nothing is **DEPLOYED-INFRASTRUCTURE VERIFIED** or **REAL DEVICE VERIFIED**. This distinction is deliberate, not an oversight, per the user's own explicit instruction not to conflate a local database with a deployed environment.

### 33.6 User actions required (unchanged in substance from §32.6, reconfirmed after this attempt)

1. Provision a real Postgres+PostGIS instance for the test backend.
2. Decide how to handle the migration-ordering defect (§33.1) before relying on `prisma migrate deploy` for a fresh database — a decision, not something this session executed.
3. Deploy the backend Docker image somewhere reachable with a real `.env.test`.
4. Register/point a real domain at it.
5. Create the GitHub `test` Environment and `TEST_BACKEND_URL` secret (§33.4 — exact steps above).
6. Run `create-super-admin.ts` against the real `.env.test` once the backend is live (or rely on the server's own boot-time seeding from `SUPER_ADMIN_*`, confirmed working this session).
7. Deploy the Admin Portal `build:test` output to a real static host; add its domain to the real test backend's `CORS_ALLOWED_ORIGINS`.
8. Once 1–5 are done, run `android-test.yml` manually.

### 33.7 Stop condition honored

No production file was modified this session — `git status --short` was clean before and after this entire pass (every file touched was gitignored and deleted before this turn ended: `backend/.env.test`, `backend/serviceAccountKey.json`, `admin/.env.test.local`, `admin/dist/`). No migration file was renamed, modified, deleted, squashed, or reordered, per explicit instruction — the defect found is documented, not fixed. Nothing was deployed externally. No production database, Firebase configuration, RevenueCat webhook, Google Maps key, or GitHub secret was touched. `android-release.yml`, `ios-testflight.yml`, and `android-apk.yml` were not modified or run. `android-test.yml` was not run, for the reason in §33.4. No iOS/TestFlight, RevenueCat purchase testing, or Google Maps key migration work was started.


## 34. Isolated Internal-Test Environment — Repository-Side Preparation (Section B): Prisma Baseline Tooling + iOS Dev TestFlight Scaffolding

Implements Section B only of the approved TEST environment plan: repository-side preparation, no deployment, no production changes, no credentials created. Apple-side setup (App ID `com.safetyalrt.alrt.dev`, "ALRT Dev" App Store Connect app, TestFlight internal group, tester) was completed by the product owner before this stage; this stage adds the repository code that targets it.

### 34.1 Prisma TEST database baseline tooling

New `backend/scripts/provision-test-db.sh` — a repeatable, documented first-time provisioning procedure for a persistent TEST database, built on the sequence verified locally in §33: `prisma db push --skip-generate` (builds the schema directly, bypassing migration replay) followed by `prisma migrate resolve --applied <name>` for every existing migration folder, run in the corrected dependency order (the two `20250317...` migrations positioned immediately after `20251004073622_init`, matching their true March-2026 authorship date rather than their misdated filenames — see §33.1 for the full root-cause investigation). No migration file is renamed, reordered, edited, deleted, or squashed by this script or by anything in this stage.

Three safety guards, each verified this stage:
- Refuses to run without `.env.test`'s `NODE_ENV=test` (verified: refuses cleanly against a `NODE_ENV=prod` file).
- Refuses to run without an explicit `--yes` flag (verified).
- Refuses to run against a database that already has rows in `_prisma_migrations` — a best-effort check via `psql` when available — since `db push` can alter live schema on a database that has already diverged (verified: refused correctly against an already-baselined database).

**Empirically re-verified end-to-end this stage** (not just re-asserted from §33's proof): ran the script against a fresh disposable local Postgres+PostGIS database — all 101 migrations resolved with zero failures, `prisma migrate status` reported the database up to date, and a genuine new dummy migration applied cleanly afterward via ordinary `prisma migrate deploy`, confirming the database is left in a state where all future migrations apply normally. Database dropped, all temporary env files deleted, Postgres service stopped before this stage ended.

### 34.2 `docker-compose.test.yml` — first boot vs. steady-state separated

The file's `app` service command (`prisma migrate deploy && yarn start`) would fail on a truly empty database for the reason above — a real defect in the file as it stood, now corrected. Added a new `migrate-baseline` service, gated behind Docker Compose's `profiles: ["baseline"]` so it **never** runs on a plain `docker compose up` — it must be invoked explicitly, exactly once, via `docker compose -f docker-compose.test.yml --profile baseline run --rm migrate-baseline`, which runs `scripts/provision-test-db.sh --yes` inside a container built from the same, unmodified image. The `app` service's own command is unchanged — once baselined, `prisma migrate deploy` on every normal start behaves exactly like production's `docker-compose.yml` already does (applies only genuinely new migrations). `docker-compose.yml` (production blue/green) and `docker-compose.dev.yml` were not touched.

`backend/docs/TEST_ENVIRONMENT.md` updated to reflect the corrected first-boot sequence and to document the migration-ordering defect and baseline rationale inline, so the procedure is followable without cross-referencing §33.

### 34.3 iOS Dev TestFlight — additive scaffolding only, nothing run

Three new/additive changes, targeting `com.safetyalrt.alrt.dev` / the "ALRT Dev" App Store Connect app:

| File | Change |
|---|---|
| `frontend/.github/workflows/ios-test-testflight.yml` | New. `workflow_dispatch`-only, `environment: test`, guarded on `secrets.TEST_BACKEND_URL` (same pattern as `android-test.yml`). Builds `--flavor dev -t lib/main_dev.dart`, `DEV_BASE_URL` sourced from the isolated test backend's secret, `ALRT_PLUS_TEST_UNLOCK=true`. Signs via the new `ci_certs_dev` lane and uploads via the existing, unchanged `ci_upload` lane (which resolves its target App Store Connect app from the IPA's own embedded bundle ID, so it needed no change). |
| `frontend/ios/fastlane/Fastfile` | Additive only — new `ci_certs_dev` lane, a copy of `ci_certs` with `bundle_identifier: "com.safetyalrt.alrt.dev"`, `profile_name: "match AppStore com.safetyalrt.alrt.dev"`, and an explicit `app_identifier:` override on the `match()` call (the shared `Appfile` hardcodes `com.safetyalrt.alrt` as its default — confirmed by reading it this stage — so the dev lane must override it explicitly, which it does). The existing `ci_certs`/`ci_upload` lanes are untouched — confirmed via `git diff`, zero removed lines. |
| `frontend/ios/ExportOptionsCI-dev.plist` | New. Same shape as `ExportOptionsCI.plist`, `provisioningProfiles` mapping `com.safetyalrt.alrt.dev` → `"match AppStore com.safetyalrt.alrt.dev"`. `ExportOptionsCI.plist` itself is untouched. |

**Not run.** `ci_certs_dev` requires a `match`-generated distribution profile for `com.safetyalrt.alrt.dev`, which does not exist yet — the first real run of `ios-test-testflight.yml` must pass `generate_certs: true` to create it (additive to the existing `MATCH_GIT_URL` certs repo; does not touch the production certificate/profile). This is a real action against shared signing infrastructure and was explicitly not taken this stage — it requires you present to watch the run, per the plan's own risk note.

### 34.4 Android

No changes. `android-test.yml` (Stage A) already implements the approved Option A (sideloaded dev APK via GitHub Actions artifact) and needed nothing further for this stage. `android-release.yml` was not touched.

### 34.5 Verification

- `npx tsc --noEmit`: clean except the single pre-existing `serviceAccountKey.json` import error (documented baseline exception, `backend/CLAUDE.md`) — no backend source file was changed this stage, so this is unaffected.
- YAML syntax validated (`python3 -c "import yaml; ..."`) for `docker-compose.test.yml`, `docker-compose.yml` (prod, confirmed unchanged), `docker-compose.dev.yml` (confirmed unchanged), and `ios-test-testflight.yml`.
- Ruby syntax validated (`ruby -c`) for the modified `Fastfile` — clean.
- `provision-test-db.sh` end-to-end local proof, detailed in §34.1, including all three safety guards individually triggered and confirmed correct.
- `git diff --stat` against every forbidden path (`backend/src/`, `backend/docker-compose.yml`, `backend/docker-compose.dev.yml`, `android-release.yml`, `ios-testflight.yml`, `frontend/lib/`, `ExportOptionsCI.plist`, `Runner/GoogleService-Info.plist`, `backend/prisma/migrations/`) — zero diff on every one.

### 34.6 Manual actions still required (unchanged in substance from §33.6/prior iOS plan, reconfirmed)

1. Provision the isolated TEST Postgres+PostGIS instance and backend host; run `scripts/provision-test-db.sh --yes` against it once.
2. Register/point DNS for the test backend's real domain (`api-test.safetyalrt.com` proposed — still does not resolve, confirmed by direct lookup two turns ago).
3. Create the GitHub `test` Environment and `TEST_BACKEND_URL` secret.
4. Deploy the Admin Portal `build:test` output to a static host; add its domain to the test backend's `CORS_ALLOWED_ORIGINS`.
5. Run `android-test.yml` and (once its cert prerequisite is met, watched live) `ios-test-testflight.yml` manually.
6. Decide real-vs-fake `serviceAccountKey.json` for the TEST image build (§ prior stage's plan, item 7) before first deployment.

### 34.7 Stop condition honored

No production file was modified — confirmed by `git diff --stat` against every path in the stop condition, all zero. No migration file was renamed, edited, reordered, deleted, or squashed. No credential was created, rotated, or deleted. Nothing was deployed. `ios-testflight.yml`, `android-release.yml`, `docker-compose.yml`, `docker-compose.dev.yml`, `ExportOptionsCI.plist`, and the existing `ci_certs`/`ci_upload` Fastlane lanes are all unchanged. All temporary local database/env artifacts created for verification were deleted before this stage ended.


## 35. Fix: Hardcoded Production Share URL in DEV/TEST Builds

Follows the TEST isolation audit that preceded Stage 34's approval — that audit found one confirmed, unfixed bug: `share_alert.dart` built the public alert share-sheet link with the hardcoded production constant `kUrlBase`, bypassing the flavor-based `providerOfBaseUrl` override entirely. Fixed this stage, with explicit approval.

### 35.1 The bug, precisely

`shareAlert()` computed `final url = '$kUrlBase/a/$id';` — `kUrlBase` is `'https://api.safetyalrt.com'`, a top-level constant in `endpoints.dart`, read directly rather than through the app's actual base-URL provider. Every other network path in the app (REST via Dio, Socket.IO via the same Dio instance's `baseUrl`) is correctly gated by `providerOfBaseUrl`, which `app_bootstrap.dart` overrides to `providerOfBaseUrlDev` for `AppFlavor.dev` — this one function was the sole exception, confirmed by a repo-wide grep for `kUrlBase` and for the literal string `api.safetyalrt.com` before this fix (both matched only `endpoints.dart` and this one call site).

**Effect before the fix**: a DEV/TEST build's "Share" action generated a link like `https://api.safetyalrt.com/a/<id>` — production's public share page — for a hazard ID that exists only in whichever backend the DEV build actually talks to (in production today, the same production backend the rest of the app already used; once the isolated TEST backend exists, a hazard created there). Not a data-write or credential leak — no automatic network call was made — but a broken/wrong link a real person could tap and share further.

### 35.2 The fix

- `share_alert.dart`: `shareAlert()` now takes a `required WidgetRef ref` parameter and reads `ref.read(providerOfBaseUrl)` — the same provider every other network path already uses — instead of `kUrlBase`. The URL-construction itself was pulled into a new, pure, exported function, `buildAlertShareUrl({required baseUrl, required hazardId})`, kept separate so it's unit-testable without a provider/widget harness (matching this repo's existing test style, which has no Riverpod `ProviderContainer` precedent to build on). The `kUrlBase`/`endpoints.dart` import is removed from this file entirely — no longer referenced.
- Three call sites updated to pass `ref:` — `view_hazard_screen.dart` (×2, both already inside `ConsumerState`/`Consumer` contexts with `ref` in scope) and `common_hazards_list_item.dart` (×1, inside an existing `Consumer` builder). No new provider, no new environment-variable mechanism, and no change to `providerOfBaseUrl`/`providerOfBaseUrlDev`/`kUrlBase`/`kUrlBaseDev` themselves — this fix routes an existing bypass through the existing mechanism, nothing new was introduced.

### 35.3 Verification

No Flutter toolchain exists in this environment (unchanged limitation, every prior frontend stage) — `flutter analyze`/`flutter test`/`dart format` could not be run directly. Performed instead:
- Full manual read-back of every changed file.
- A bracket/brace/paren balance check across all four changed/new files (`share_alert.dart`, `view_hazard_screen.dart`, `common_hazards_list_item.dart`, the new test file) — all balanced.
- Repo-wide grep confirming exactly three `shareAlert(` call sites exist, all three now pass `ref:`, and `api.safetyalrt.com` appears nowhere in `lib/` except its single canonical definition in `endpoints.dart`.
- New test file `test/share_alert_url_test.dart` — three cases for `buildAlertShareUrl`: production base URL, the TEST backend URL (`api-test.safetyalrt.com`), and the local-dev-server fallback URL — matching this repo's existing pure-function unit-test style (`band_colour_lock_test.dart` etc.), asserting the function reflects whatever base URL it's given rather than a hardcoded one. Not run (no Flutter toolchain), but syntactically verified.

**CODE VERIFIED**, not **REAL DEVICE VERIFIED** — same distinction this project has drawn throughout, since no Flutter/Android/iOS toolchain has ever been available in this environment.

### 35.4 Confirmed behavior

- **Production**: `providerOfBaseUrl` is unmodified — `AppFlavor.prod` still resolves it to `kUrlBase` (`https://api.safetyalrt.com`), so `buildAlertShareUrl` still produces `https://api.safetyalrt.com/a/<id>`, unchanged from before this fix.
- **DEV/TEST**: `providerOfBaseUrl` is overridden to `providerOfBaseUrlDev`, which resolves to `DEV_BASE_URL` (`TEST_BACKEND_URL` at CI time, e.g. `https://api-test.safetyalrt.com`) when set, or the local-dev fallback otherwise — never production. `buildAlertShareUrl` now reflects whichever of these is actually active.
...
**Build 41** (version 1.0.5, build 41; build 40 was cancelled before it finished, superseded by this one) adds handling for several check-in asks at once, from the approved "Check-in requests" mockups. Before it, the hub only ever held the latest ask concerning the member, so when Amy and Tom both asked within a day only Tom was named; one check-in did answer both (a check-in counts against every ask made before it), but the person could not see that. Now: the backend returns every open ask from the last day that concerns the member (`checkInRequests`, newest first; `latestCheckInRequest` is kept for older app builds) and a per-circle `pendingCheckInRequests` count in the circle list; the hub's white check-in card shows the askers' faces, "Amy, Tom +2 requested a check-in", when, "View N requests", the green **Check in · lets Amy, Tom +2 know** button and "Location sharing is optional"; the "View N requests" sheet lists each asker with time and message, other circles with open asks ("Switch circle"), and one **Check in to <circle>** button; the Choose-a-circle sheet says "1 check-in request waiting on you" on such a circle. The old amber "asked you to check in / Tap Check in below" strip is gone for the people asked; the asker's own amber tracker (who has answered, nudge, cancel) stays. No schema change and no migration. Also in this build: the consent buttons read exactly "Check in and share my suburb" / "…my location" (the stray "too" is gone), the backend warns at startup when `GOOGLE_MAPS_API_KEY` is not key-shaped (39 characters starting with AIza), and rollout script revision 5.1 stops the deploy stage when that warning appears in the started app's log. **Revision 5.1 could not deploy an update**: its migration gate was written for the first rollout only (it rejected "database schema is up to date", required the two approved migrations to be pending and required zero approved columns), so on TEST, where both migrations are finished and all three columns exist, the deploy stage would have stopped at the gate. Found by Codex review before any command was run on TEST. **Revision 6** keeps that first-rollout gate unchanged behind `--apply-approved-migrations` and adds an explicit `--no-new-migrations` mode for updates: `prisma migrate status` from the new image must exit 0 and report the schema up to date with nothing pending; the ledger must be clean with both approved migrations finished; all three columns must be present; the image's migration folders must equal the ledger; the start must apply nothing (ledger names identical before and after, no "Applying migration" line). Rollback tag, exact image ID, fresh restore-tested backup and stop-on-first-failure are shared by both modes; nothing resets, re-applies or rolls back. **Revision 6 failed on TEST at 10:43Z (run `run-20260908104347-4178407`, pin 539a154)**: the app image built ("naming to docker.io/library/backend-app done") but the script took the first line of `compose config --images app`, which lists the app's dependencies first, so it inspected `postgis/postgis:15-3.4`, saw a creation date before the build and stopped. Nothing was started; the TEST app kept running on `60c5709…`; the checkout advanced to 539a154 and the new app image exists. **Revision 7** identifies the app image from the app service itself (its explicit `image:` if declared, else compose's `<project>-app` with the project name read from `compose config --format json`, cross-checked by membership in `--images app` and by the build log's own naming line), then proves the image is the pinned checkout by hashing the six content files inside it against the checkout, and checks its compose service label. Creation time is recorded, not judged, so a fully cached image from an earlier attempt deploys when its content matches. Proven by the stand-in harness against the real compose shape (postgres listed first, the baseline service listed, cached build, missing image, mismatched content, a build naming a different image). The targeted-ask regression script gained a §8 with four multi-ask checks (every open ask newest first, an ask aimed at someone else excluded, pending counts per circle, one check-in after the newest ask clearing them); it passes locally against a real PostgreSQL (14 checks) and runs on TEST inside `verify` with the strict 40-check consent suite and the other five regressions, all against the new image. Proven by the stand-in harness (13 new update-path scenarios: no mode, both modes, wrong mode either way, status failure, unreachable database, dirty ledger, ledger/schema disagreement, ledger/image folder mismatch, a migration applied during an update, restore failure, the good path, verify after it) and by the real-PostgreSQL harness (update refused while the two migrations are pending; after the first rollout, first-rollout mode refused, update deploy passes with the ledger byte-identical before and after, no scratch database left). **Build 42** (commit after 1230728, frontend only): a spontaneous check-in no longer fans out to a circle where someone is waiting on that member's check-in; answering an ask is always the member's own tap in that circle (Switch circle, then Check in). With an ask owed in the open circle the check-in already went to that circle only. Pure helper `checkInTargetCircleIds` with four unit tests. TEST backend deployed at 1230728 on 8 Sep 12:30Z (run-20260908123008-15778): 40/40 consent, six regressions 6+8+13+14+5+5 = 91 checks, health OK, migrations finished, columns present, no restarts, backup restore-tested, scheduler OFF. Build 41's multi-ask list needs the TEST backend redeployed at this commit or later; until then the app falls back to the single newest ask. Backend `tsc` clean; `flutter analyze` clean; `flutter test` green.

**Build 43 (9 September 2026, phone acceptance round 1, six issues).** Root causes and fixes, all in the app unless marked backend:

1. *SOS navigation and the duplicate sender screen.* The send screen stayed on its tick; now, once the server confirms, the sender returns to Family after about a second (Done sooner), where the red "Your SOS is active" banner and Open already live. A send whose answer was lost asks the server for my active SOS before offering a retry, so a retry can never raise a second one. Standing down no longer pushes the recipients' after-event page onto the sender; the sender gets one toast on Family, and the recipients' page stays reachable from SOS history. The live SOS page reads "Your SOS" / "Your SOS has ended" for the person in SOS, and names the sender for everyone else (the sender had seen "Family member is marked safe"). Recipients are never navigated by an SOS ending. The generic "Family member" wording appears only when an account has no name set, which is the state of the current TEST accounts.
2. *Leaving a circle hung.* Leaving wiped the state to "no circles"; the next background refresh repopulated the circle list while nothing was in scope, and the tab sat on its spinner. Leaving now drops the departed circle and loads the next one (or the no-circle screen); a refused leave keeps the circle and reports the error; a leave whose answer was lost is recognised by asking the server which circles remain, never retried blindly. Four provider tests.
3. *Notification settings vs filters* (trace in the harness agent report, summarised in TEST_BUILD_44_ACCEPTANCE.md). Fixed: the push switches now use the same names as the filters (Emergency Warning, Watch & Act, Advice, Official, Community); the map and the feed share ONE persisted filter state (the feed and the map had separate, transient copies); the Map details sheet's second, unsynced set of source toggles is replaced by a link to ALRT Filters; **backend**: an account with no push-settings row was answered "everything off" and matched no push at all, now the row is created with the real defaults (every type on, every main category); **backend**: Emergency Warnings are exempt from the per-user push cooldown. Left as documented gaps needing approval: push has no Global humanitarian / ALRT Intel switch (they push under Official), the source push policies (after-confirmation, green-exempt) are not enforced, onboarding's three-way choice cannot express the five switches.
4. *Map details* opens at three-quarters with the map visible, internal scroll and Done pinned; light background (owner, 9 Sep; build 44).
5. *Check in from every alert.* The strip lived inside "What to do", so alerts without directives (every community report) had no way to tell the circle you are okay. It now sits on every alert detail, says which circle it goes to (change circle when there are two), links the alert id on the check-in, and tells a user with no circle where to go.
6. *Vibration.* The urgent Android channel was created once with the strong pattern regardless of the switch, and background pushes named no channel, so they always landed on the normal channel. Now the **backend** names `alrt_alerts_urgent` on take-action and critical pushes (high priority) and the app creates that channel only while the switch is on and deletes it when off, so the choice holds for background pushes too; a "Test vibration" row posts a real notification on that channel and says when Android has notifications for ALRT blocked. Not verified on hardware from here.

**Build 44 (9 September 2026, commit ba953d6)** is build 43 (1aee3d4) plus the light Map details sheet, nothing else: run 34296314173 (test_store, artifact 10083518128, zip sha256 `3e6d3d51491133ec3f29fb6665bc1b2d0f902a79d874f4d3e4dd3ca4e17f285a`) and 34296315909 (bypass, artifact 10083592567, `fc7a04de4a30a6be1b82a91cd0330353b54912022c19786e10271314bc3ff8fe`), both with the TEST keystore SHA-1 verified by apksigner; footer `ALRT 1.0.5 (44) · TEST build · ba953d6 · RevenueCat Test Store` (or `billing bypass`). The Android APK itself is unchanged by the commits after ba953d6 (backend script, rollout script, tests and documents only).

**Notification behaviour, targeted backend verification (after ba953d6).** `src/scripts/verify_push_notification_settings.ts`, run by the rollout's verify stage as the seventh regression script (rollout revision 8; also a content-verified file together with `notification.service.ts` and `user.service.ts`), against the real HTTP API and the real send path with Firebase intercepted at `sendEachForMulticast`: (1) an account with no settings row gets every type on and every main category, exactly one row is created and a second read returns it untouched; (2) opt-outs are never overwritten: switches saved off through the API, and a pre-existing all-off row, come back exactly as saved from the API and from the service behind it, with `updatedAt` unchanged; (3) delivery honours the switches: with Advice, Official and Community off the user still receives an Emergency Warning and a Watch & Act push and receives no Advice, Official (non-AWS) or Community push, while an all-on user in the same area gets all of them; (4) every hazard push is high priority and names `alrt_alerts_urgent` only for the action and critical bands, `alrt_alerts` otherwise, including a critical-band official non-AWS alert; (5) with a cache connected, a user at the cooldown limit gets no Advice push but still gets an Emergency Warning; without `CACHE_URL` that section prints **skipped**, never passed. Local results: 14 passed + 1 skipped without a cache, 16 passed with a local Redis; a mutation run (exemption removed, channel fixed to `alrt_alerts`) fails at the first urgent-channel check. The app side: `accessible_alerts_preference_test.dart` (5 tests) pins that the strong-vibration choice lives in device storage, an explicit choice beats the safety-profile default and a later cohort change, and off/on/on ends on; the channel create/delete calls and the vibration itself need a phone. Rollout revision 8 passes the stand-in harness and the real-PostgreSQL harness. **Not done and stated as such**: Global humanitarian and ALRT Intel still push under the Official switch (a separate switch needs two new columns on `UserPushNotificationSetting`, a migration, and approval), the source push policies (after-confirmation, green-exempt) are not enforced on delivery, and onboarding's three-way choice cannot express the five switches. **TEST server status**: still at 1230728; none of the build-43/44 backend changes (push defaults, Emergency exemption, urgent channel name) or this script have run on TEST yet.

**Build 45 (9 September 2026, phone acceptance round 2, seven issues).** Source revision at the start of this round: `test` at f665163 (build 44's app source ba953d6 plus backend scripts and documents); build 44 had not been superseded (runs 48/49 were the newest). The installed build and billing mode are read from the Profile footer, never from appearance. Root causes and fixes (app unless marked backend):

1. *SOS events (beyond build 43's navigation fix).* The 4-hour lapse sent a bare `{id, circleId, status}` that the app's parser rejected, so the red strip stayed live on every receiver until a reload (**backend** now sends the full row; the app also tolerates the bare shape). A replayed or late `familySos` re-inserted an ended SOS and showed its banner again (the provider now remembers ended ids and shows one banner per id). A banner or push tap opened the payload captured at delivery, so a resolved SOS could render as active (the screen now asks the server and shows "ended" when it is not listed). A tap on an SOS push landed on the hub (it now opens the SOS if still live). The active list was single-circle and every reload wiped another circle's live SOS (**backend** `GET /family/sos/active` without circleId now returns every circle the user is in). Name fallback unified to "Family member" / "A family member" (was also "Someone", "A member", "They"). Foreground: a push and the socket both announce the same SOS once each; the in-app banner is de-duplicated by id.
2. *Notifications, map and feed.* The map ignored the Global humanitarian and ALRT Intel filter switches (only the feed applied them) and keyed AWS levels on the derived band instead of the official severity the server filters on; both aligned. **Backend**: the AWS info/unknown levels were pushed under Advice but never returned by the list; the Advice filter now returns them too (the official level word on the card is unchanged). Matrix script (56 checks) added; the table is in the acceptance guide. Still open and needing decisions, listed below.
3. *Map details*: light three-quarter sheet, unchanged from build 44 (screenshot scene 15).
4. *Check in from every alert.* The alert id was stored but never validated or shown: **backend** now refuses an id that is not a published alert (400, uuid-checked) and returns the alert's title with every check-in; the member sheet shows `Checked in near "<alert>"` and the message.
5. *Leaving a circle.* **Backend**: the leave endpoint said only `{success: true}`; it now reports `left`, `hostTransition` or `deleted`, the last member out (host or not) deletes the circle so no memberless circle is left behind, and the leaver's own other devices are told. App: toast per outcome; socket events for a departed circle (SOS, check-in, proximity) are ignored; SOS lists and scheduled check-ins are cleared and reloaded.
6. *Vibration*: unchanged from build 43; preference tests from the build-44 handover; hardware acceptance separate.
7. *Saved-location paywall.* Trace: the only save entry points are the Search tab's three Subscribe buttons (Manage locations has no Add; the map has none). Free allowance 1 in the app (`kFreeSavedLocationsLimit`) and 1 in the backend (`FREE_SAVED_LOCATIONS_LIMIT`), no conflict; the older docs' "paywall only at hosting" wording is stale. Server enforcement of the limit is OFF on TEST (`BILLING_ENABLED=false`), so the gate is client-side only, stated as such. Silent dead ends fixed: the entitlement check could throw and swallow the tap; the saved count read 0 before the list loaded; the upsell sheet needed a navigator context that could be null; a server 403 was never shown; a cancelled store dialog was reported as a failure; an offering whose packages are not named monthly/annual rendered no plan cards; the load failure had no retry and no distinction between "no billing key" and "could not load". Now: one shared tap handler with an explicit outcome (saved, removed, needs ALRT+, could not check, failed) and a toast for each; the paywall opens with a saved-location headline, reads real error codes, offers Try again, refreshes every entitlement provider after purchase, and the pending save completes on the same screen. Not automated here: the RevenueCat offering and a Test Store purchase. The workflow now prints the APK's sha256 and apksigner's scheme report so the downloaded binary can be checked on a computer.

Rollout script revision 9 runs nine regression scripts (the two new ones content-verified with `hazard.util.ts` and `family.validator.ts`); both harnesses pass. Automated results at this commit: `flutter analyze` clean (5 pre-existing infos), `flutter test` 241 passed / 15 skipped, backend `tsc` clean, verify scripts locally 12 + 56 + 16 (with a cache) checks, screenshot harness 18 scenes. No migration or schema change; deploy mode `--no-new-migrations`. TEST server still at 1230728 until the build-45 pin is rolled out.

**Decisions needed before issue 2 can be called complete (no schema or policy change made without approval):**

- (a) A push switch for Global humanitarian and one for ALRT Intel: two boolean columns on `UserPushNotificationSetting` (default true), one migration, the push gate reading them, two switches in Manage notifications. Until then they push under Official.
- (b) Enforcing the seeded source push policies on delivery: `afterConfirmation` (community reports push only once accepted, including admin acceptance, which today sends no push at all), `greenExempt` (GDACS Green never pushes), `advisoryOnly`/`bandThreshold` (a per-source floor, the standard's "notify at ACTION and CRITICAL only" default). Code only, but it changes which pushes go out.
- (c) Onboarding's three-way choice: "User reported alerts" currently turns all four official switches off, including Emergency Warning. Proposed: Emergency Warning stays on whatever is chosen, and the step is reworded to the five switches (or a fourth option). Product wording decision.

**Build 46 (9 September 2026, issue 7 continued).** Added on top of build 45, after the owner listed the five paywall acceptance points: (1) the RevenueCat SDK is now behind a small gateway so the account rules are tested: the SDK identity follows the ALRT user id, an account switch signs the SDK in as the new id before any read, a switch that fails (offline) reads nothing rather than the previous person's entitlement, every entitlement read is checked against the account it is for, and signing out of ALRT signs the SDK out (it never did before, so the next account on the phone could have read the previous one's entitlement until its own sign-in completed); (2) `saved_location_flow_test.dart` runs the five acceptance points through the real provider with the server and store faked; (3) `verify_saved_location_limit` proves the server side in both modes and states which one it ran in, so "the limit is enforced" is never claimed on a server with `BILLING_ENABLED=false` (TEST today); (4) the acceptance guide carries the issue-7 phone checklist and names the exact configuration the paywall depends on (a current RevenueCat offering with packages on the `plus` entitlement in the TEST project, and the TEST Google key secret), which needs its own approval and is not something a rebuild can fix. Rollout revision 10 (ten regression scripts). Build 44's paywalls remain unverified; nothing about issue 7 is marked complete until the phone checklist is run. The failure point on the owner's phone is still unknown from here: the one question asked is which screen appears on the second Subscribe tap.

**Build 47 (9 September 2026, twelve items: the outstanding phone fixes, the widget, and notification delivery).** State first: the owner's phone had build 46 or earlier (the Profile footer is the only proof); the TEST server was verified at **6a1115f** (the build-46 pin) on 9 September 2026 (run `run-20260909042910-293693`, image `sha256:95fe66e1…a65cb`, 40/40 consent checks, ten regression scripts, health, migrations and columns confirmed, scheduler OFF), so everything from builds 43 to 46 is live and only the build-47 backend delta is not; nothing below is claimed as phone-verified. (An earlier draft of this paragraph wrongly said TEST was still at 1230728; corrected.)

1. *SOS returns to Family (issue 1), reproduced through real navigation.* `family_sos_screen_navigation_test.dart` pushes the actual send screen on a GoRouter, holds the button for three seconds on a 360×780 phone surface and checks where the app is afterwards. The first run overflowed by 61 px on that surface, which is the real small-phone layout bug: the send screen's column had no scroll view, so on a short phone or with large text the Done/Cancel button sat below the screen and the layout threw. The body now scrolls when short and keeps the hold button centred when tall. Other causes fixed in the same pass: the Family-tab provider was auto-disposed, so the tab choice made by the send screen was lost as it left (now kept alive); the auto-return timer could navigate a second time after Done (cancelled on Done and on dispose, one navigation guarded); a second hold while the first was in flight could raise a second SOS (guarded); the home screen re-applied its initial tab on every rebuild (initial tab now optional). A refresh that raced the send could end the sender's own SOS locally (90-second grace, for SOS this phone raised only: a socket copy gets none, because the server already knows it, and a refresh that omits it is the server saying it ended).
2. *No recipient-facing screen for the sender (issue 2).* "Is this mine" is now decided from the member ids of every circle the account is in, not the open circle only, so a stand-down from another circle never reads as someone else's; the receiver screen keeps the sender's wording while the stand-down request is in flight; a failed stand-down says so and leaves the SOS live; the circle payload no longer overwrites the cross-circle active list on every load. The provider now cancels its periodic timers (SOS live share, stale poll, journey points) on dispose, so sign-out and account switches stop every stream.
3. *Home-screen widget.* Rebuilt on a pure model (`FamilyWidgetModel.build`): one row per circle, ordered SOS › check-in requested (owed by me) › waiting for responses › ordinary, honest wording ("No pending requests", "1 of 2 checked in today"), words plus glyph plus colour, no member name, location or safety-profile detail in the payload, one deep link per row that only ever opens the app (circle, check-in flow or that SOS; never a silent action), freshness line ("Updated h:mm", "As of h:mm · open ALRT" after an hour), "+N more circles" when the card is full, signed-out and no-circle states. Android layouts and provider rewritten for rows; iOS Swift updated but the widget target is still not in the Xcode project (blocked). Previews for six states plus the stale marker, normal and large text, drawn from the real payload; on-device rendering not observed.
4. *Map details*: one implementation (`MapDetailsSheet`), the Keys button, the dark keys sheet and the duplicate visible-systems provider deleted; rail badge and marker filter read the same filter provider as the feed.
5. *Check-in strip*: `ViewHazardScreen` is the only alert renderer, every route lands there, so all five kinds carry the strip; a loading state replaces the wrong "no circle" while the circle loads; tests render each kind with a complete screen and prove no auto send.
6. *Check-in visibility / map privacy*: unchanged rules, selected-circle scope preserved.
7. *Notification settings alignment*: unchanged from build 45; the three decisions (a)–(c) above are still open and still listed in the guide.
8. *Leave a circle*: build 45 fixes; widget rows for the departed circle drop on the next payload; stale socket subscriptions ignored by circle scope.
9. *Vibration*: `urgent` now travels in the push data and the local notification honours it, so the strong pattern is predictable from the server's choice; the settings card names a denied permission and opens the phone's settings; channels are never deleted and recreated to override a user's choice.
10. *Saved-location paywall*: build 46 flow plus a re-entrancy guard (two quick taps save once).
11. *ALRT+ benefits*: one table (`alrt_plus_benefits.dart`) on the paywall, the manage screen and the expired screen, limits from one file (1 saved location free, 4 hosted circles, 8 seats, joining free); prices, periods and trials come from the store only.
12. *Notification delivery.* **Backend**: hazard pushes go through `pushSafeHazard()` (no coordinates, bbox, reporter or review fields in any push body); family pushes carry `urgent` (SOS start and needs-help urgent, check-ins and stand-downs ordinary) and bodies without free text or place names; Android visibility private, APNs sound and interruption level set; dead tokens pruned on `UNREGISTERED`; `POST /api/notifications/test` and `DELETE /api/notifications/push-notification-token` added. **App**: token refresh re-registers; sign-out deletes the token on the server first (4 s cap); a push tap is consumed once per message id; stable notification ids per event so a duplicate delivery replaces rather than stacks; the Manage notifications card shows the permission state, opens the phone's settings and sends a clearly labelled test notification. `verify_family_push_delivery` (11 checks) proves eligibility and the message shape for SOS start, stand-down, check-in request and response, the sender excluded, a departed member and a signed-out token getting nothing, and the test endpoint. Delivery, lock-screen display, sound and vibration remain untested until observed on a phone; the guide carries the per-event, per-state recording table. Known and unfixed: a public alert can arrive twice when it is both in a saved area and near a family member; no background-message handler (benign while every push has a notification block); iOS `aps-environment` production in dev entitlements; no iOS time-sensitive entitlement; account deletion does not clear the widget.

Rollout script revision 11 (eleven regression scripts, `verify_family_push_delivery` content-verified with the notification controller, route and family alert service); both harnesses pass. Automated results at commit 01489ad: `flutter analyze` clean (7 pre-existing infos), `flutter test` 288 passed / 17 skipped, backend `tsc` clean, verify scripts locally 11 (push delivery) + 15 (+1 skipped, settings) + 12 + 56 + 3/5, screenshot harness 20 scenes. No migration or schema change since 6a1115f; deploy mode `--no-new-migrations`.

**Build 47 backend delta, second pass (9 September 2026, after the owner's baseline correction).** Backend only; the build-47 APK (01489ad) is unchanged and remains the candidate.

1. *Duplicate public-alert delivery, fixed.* One accepted hazard reached a person once per qualifying path: the saved-area path (`viewHazard`) and the family paths (`familyHazardProximity`: one `notifyCircle` per nearby member and per nearby saved place). `claimHazardPushRecipients(hazardId, eventKey, userIds)` in `notification.service.ts` now grants one push per person per hazard *event* across all of them (Redis `SET NX EX` for 48 h, in-process map when there is no cache; the creation event's paths all run in one process so the fallback covers it). The saved-area query claims its recipients before flattening tokens; `notifyCircle` takes `hazardEvent` and claims before pushing, while the socket event still reaches everyone. The event key comes from the hazard's `createdAt`, so a genuine later event uses a new key and is not suppressed; `releaseHazardPushClaims(hazardId)` exists for a deliberate re-issue (and for the matrix script, which pushes one hazard under many settings). Every hazard push also carries `collapseKey` (`android.notification.tag`, `apns-collapse-id`), so a copy that still arrives replaces rather than stacks in the tray. `verify_hazard_push_dedupe` (7 checks): a person in a covering saved area and in a circle with two nearby places gets exactly one push, circle-only and area-only people get one each, a retry of the creation paths sends nothing, a new event key claims afresh while the creation claim stays, tray keys present, socket fan-out untouched; passes with and without a cache.
2. *Notification handling by payload type and app state, traced.* Every send carries both a `notification` block and `data` (`notificationType`, `payload`, `urgent`); there is no data-only push. Foreground: `onMessage` → the app draws it (stable id per event, urgent channel when marked) and a tap routes by type. Background: the system draws the `notification` block on the server-named channel; tap → `onMessageOpenedApp`. Closed: system draws; the launching tap is read once via `getInitialMessage` (consumed-id guard). Locked: system draws with private visibility. Offline then online: FCM holds and delivers; an SOS tap asks the server first. **The missing `onBackgroundMessage` handler prevents no required behaviour**: it only matters for data-only pushes, and one that drew a local notification would duplicate the system's copy. Not added.
3. *iOS, explained against the real build path* (`ios-test-testflight.yml`: manual signing via fastlane match, `ExportOptionsCI-dev.plist`, TestFlight "ALRT Dev"; GitHub has never registered a run, so no iOS TEST build exists). `aps-environment = production` in the dev entitlements is correct for a TestFlight build, not a defect. Time-sensitive delivery needs the App ID capability, the entitlement in both files and a regenerated profile: a signing/capability change, **blocked pending approval**. The widget target needs Xcode, the `.dev` App Group on the portal and a second match profile: **blocked**. Production signing and capabilities untouched.
4. *RevenueCat TEST offering*: no read access from here (no key of any kind in this environment; the backend holds only the webhook secret; the REST API needs a secret key). The guide lists the six dashboard screens to screenshot. No APK or mocked test is offered as evidence of the configuration.
5. *Policy decisions (a)–(c)* remain pending; no migration, no opt-out change.

Rollout script revision 12 (twelve regression scripts); both harnesses pass. Automated results at this commit: backend `tsc` clean; scripts locally: dedupe 7 (with and without a cache), push delivery 11, settings 17, alert filter matrix 56 (with and without a cache). Frontend unchanged from 01489ad.

**Revision 12 failed on TEST at 20:46Z (run `run-20260909204606-578861`, pin 0f07831).** The image built (`sha256:214c797b…4c2b`), the app service resolved to `backend-app`, and the stage then ended with exit 1 and no STOP line. Cause, confirmed by reproduction: the build-log cross-check ran `grep -oE 'naming to (docker\.io/library/)?[^ ]+ done'`, which requires the image name directly before " done"; the host's BuildKit prints `naming to docker.io/library/backend-app 0.0s done`, so grep matched nothing and exited 1, and under `set -e` with `pipefail` the assignment `named=$(...)` ended the script. Nothing was started; the running app stayed on `sha256:95fe66e1…a65cb` with zero restarts; the failed run directory, its rollback record and the checkout at 0f07831 are untouched. **Revision 13**: the naming line is parsed by `build_log_named_image()`, which accepts BuildKit with or without a duration or "done", the classic `Successfully tagged <image>:latest`, and the absence of any naming line (reported as "<no naming line>", never fatal, because the app image is identified from the compose service and proven by its content; the naming line is only a cross-check and a genuine mismatch is still refused). `set -E` makes the ERR trap reach functions and substitutions, and an EXIT guard names any exit that no `stop()` explained (exit code and last command, on stderr and in `status.txt`). Stand-in harness: seven new scenarios (duration form, no-duration form, tagged form, no naming line, and mismatches in the duration and tagged forms, all with the parsed name checked in `app-image-name.txt`); both harnesses pass. Image resolution, pinned-content verification, migration gate, backup and rollback record are unchanged. No manual start, no rollback, no migration, no APK rebuild for this fix.

**Build 48 (9 September 2026, paywall prices and currency).** The owner's test_store phone showed USD. Trace: every amount on the test_store paywall is `StoreProduct.priceString` / `currencyCode` from the RevenueCat offering; the only hard-coded figures were the bypass build's preview cards. USD therefore comes from the RevenueCat Test Store products, which carry one dashboard-entered price and currency for every phone (the Test Store has no storefront countries); real Google Play returns the storefront country's price in its currency, formatted by Play, which shows AUD as a bare "$9.99" on an en-AU phone. App changes: `store_price.dart` (store amount as given; ISO code appended only when the string carries no letters; period from the store's ISO-8601 period, else the package type; never an amount the store did not give), used by the plan cards, the line under Subscribe (now the selected plan's price) and the manage screen (the active product's price fetched through a new `products()` gateway method, omitted when the store does not answer); preview cards relabelled `US$` with an explicit "not store prices" caption. No hard-coded AUD, no conversion, no live price or billing configuration changed. Tests: `store_price_test.dart` (9), gateway fake extended. Verification of the AUD amount itself is a dashboard/console check listed in the guide.

**Build 49 (9 September 2026, screen-rendered price checks and three narrow-phone fixes).** `alrt_plus_store_price_screens_test.dart` renders the real paywall and manage screen on a 360 px surface against a fake store that answers like Google Play for an Australian account (AUD as a bare "$", periods P1M/P1Y, an active `plus` entitlement on the monthly product): the cards read "$9.99" / "AUD · per month" and "$99.99" / "AUD · per year", the line under Subscribe follows the selected card ("$99.99 AUD a year" then "$9.99 AUD a month"), the manage summary reads "Monthly · $9.99 AUD a month · renews 9 Oct 2026" and drops the price when the store cannot answer, and nothing overflows at 1.4× text. Rendering it found real defects that a 390 px harness had hidden: the paywall's Terms · Privacy link row overflowed by 13 px at normal text and 128 px at large text (now a wrapping layout), and three manage-screen rows clipped by 2 to 11 px (title and badge, circle name and seat count, settings row label; now flexible). Account deletion now does what sign-out does on the phone: removes the push token first, clears the Family widget, signs the store identity out and drops the Ask ALRT session and entitlement caches. Golden scenes 22 to 24 added. No backend change; the rollout pin and script checksum are unchanged apart from the branch head advancing.

**Revision 13 deployed f1f6b92 to TEST at 23:37Z (run `run-20260909233730-629825`, image `sha256:9be0d3e4…f959bd`); verify stopped at the twelfth script.** Consent 40/40 and the first eleven regression scripts passed (notification settings 15 + 1 skipped, saved-location limit 3 with server enforcement OFF); `verify_hazard_push_dedupe` failed in its setup at its first registration with HTTP 429 "Too many authentication attempts from this address". Cause, from the code: `POST /api/auth/email-password/register` sits behind `authApiRateLimiter` (express-rate-limit 8.3.1, memory store: a fixed window per client address that starts at the address's first hit and resets `AUTH_RATE_LIMIT_WINDOW_MS` later, default 15 minutes, cap `AUTH_RATE_LIMIT_MAX`, default 40; a 429 counts as a hit). Every script runs inside the app container against localhost, so all thirteen share one address and one budget. Registrations per suite, all straight-line calls: consent 3, then 2, 3, 4, 4, 4, 3, 5, 3, 3, 3, 3, 3 = **43**, so the twelfth script's first registration was hit 41 and the suite inherently exceeds the budget by three; it fails at the same point on every full run. Not a deduplication result: the script never reached its first check. **Revision 14** (no backend change, no limiter change): `verify` reads the container's own cap and window, keeps a per-script registration table, and before a script that would exceed the cap waits for the window to end (bounded by one window plus five seconds, logged and written to `summary.txt` and `status.txt`, never a retry against the limiter); `verify --resume` reuses the run's earlier verify results (consent and each script whose log and summary show exit 0 and "checks passed") into a new, stamped verify directory, first waiting out the earlier verify's window from its recorded start (or the earliest log's time for a pre-14 verify), then runs only the scripts that did not pass, then the health, ledger, schema, container and scheduler checks, and logs "verification passed (resumed): N carried, M run now". Earlier verify directories are never overwritten. Harness: the 429 on the twelfth script then a passing resume with 12 carried and 1 run, a small budget (10) that paces five times and still passes every script, and a resume with nothing earlier that runs everything; both harnesses pass. Full runs of the suite now take one 15-minute pause before the twelfth script.

**TEST verified at f1f6b92 (10 September 2026, 00:02:59Z).** `verify --resume` (revision 14) completed run `run-20260909233730-629825`: the twelve earlier passes (consent 40/40 and eleven regression scripts) carried forward, `verify_hazard_push_dedupe` 7 checks run and passed, internal and external health OK, ledger and columns confirmed, image `sha256:9be0d3e4…f959bd` running with zero restarts, scheduler OFF. Everything up to build 49's backend delta (one push per person per hazard event with tray collapse keys, the test-notification and token-removal endpoints, safe hazard payloads, urgent flags, dead-token pruning) is now live on TEST. Still not verified and not changed: the notification-settings cooldown check that is skipped without a cache (1 skipped), server-side saved-location enforcement (OFF, `BILLING_ENABLED=false`) and scheduled jobs (OFF).

**Build 50 (10 September 2026, paywall condensed).** The owner's build-49 phone photo showed the paywall carrying a callout, a five-row Free-versus-ALRT+ table (two rows of which say "always free" twice) and the plans, and asked for less. The paywall now leads with "ALRT+ adds": the three paid differences as one line each (saved locations, hosting, check-ins/SOS/places in hosted circles), then one line for what stays free, then "Compare Free and ALRT+" which expands the full table in place. Both come from the same benefit list (`isPaidDifference`, `short`), so nothing is stated on the summary that the table does not also state; Manage and the expired screen keep the full table. The photo also confirmed the Test Store formats prices as "USD9.99", left untouched by the app (letters present), and that the yearly Test Store product is set to USD 79.98, not the 99.99 in the setup notes: dashboard configuration, not app text. Tests: benefits summary (2 new), paywall toggle in the store-price screens test; goldens 22, 23 and 25. Version 1.0.5+50. No backend change.

**Build 51 (10 September 2026, Free and ALRT+ columns).** The owner asked for the condensed build-50 paywall to keep the free plan sounding generous ("alerts always free, we want everyone informed"), make ALRT+ appealing, and show Free and ALRT+ as columns. The paywall now opens with a lavender note ("Alerts are always free. We want everyone informed, always. Official alerts, the map and emergency guidance never cost anything, and joining a circle someone else hosts is free too."), then a `Table` with a label column and FREE / ALRT+ columns (ALRT+ tinted), then one line on who ALRT+ is for ("…create your family circle, invite up to 8 people, and give everyone in it check-ins, SOS and saved places"). The benefit list gained short per-column cells (`freeCell`, `plusCell`; null draws a dash, "Always" draws a tick and the word, so nothing is colour-only) and a separate seats row, so the table has six rows; the summary widget and the "Compare Free and ALRT+" toggle from build 50 are gone, and Manage and the expired screen use the same table. Every cell still quotes an enforced allowance (1 place, unlimited, up to 4 circles, 8 seats); prices and periods stay the store's. A first render overflowed the tick cells by 18 px on a 360 px phone; the tick and word now sit in a `Wrap`, and the paywall, large-text (1.4x), manage and expired screens pass the 360 px screen tests without overflow. The member sheet's guest line now reads "Guest · gets alerts and can check in · uses no seat" (the server lets a guest check in and ask others to; the old line said "alerts only"). Tests: benefits (4, rewritten for the columns), store-price screens (4), store price (11); full suite below; goldens 11, 22, 23 and 24 regenerated, golden 25 removed with the toggle. Version 1.0.5+51. No backend change; TEST stays at f1f6b92.

**Build 51 APK (test_store, commit 7eacfea).** Run 34541544658 (23:18Z to 23:29Z, analyze and `flutter test` green on the runner), artifact 10177722179 `ALRT-android-test-apk-test_store` (expires 24 September 2026), zip digest `sha256:46a6925c…f753943`, APK file sha256 `b57267e6d9a6e18e6a6e8e62ca030b0e24f0113f0bbbafc673717aaec85c77ec` (app-dev-debug.apk), apksigner: Verifies, v2 scheme only, certificate SHA-1 `18:5E:43:9B:FC:7A:99:42:0C:AD:F0:EB:A9:E1:4F:0A:EE:66:2D:DB` (same TEST keystore as builds 36 to 49), certificate SHA-256 `1596aa73…bd9dfc`. Footer: `ALRT 1.0.5 (51) · TEST build · 7eacfea · RevenueCat Test Store`. Build 50 (run 34540761715, fbe0b0c) completed but is superseded and was not handed over. Build 51 is the candidate for the paywall items; phone acceptance of every item remains open.

### 36.7 Device acceptance (physical phones, separate from the automated results above)

Build 47: no item of the phone checklists in `TEST_BUILD_51_ACCEPTANCE.md` has been run on a phone from this environment; none is reported as passing; the hardware columns (delivery, sound, vibration) and the launcher's widget rendering are untested. Build 46 and build 44 (ba953d6): likewise. Build 44 (ba953d6): likewise. Earlier: on phone A, build 36 showed the earlier header ("Your check-in is owed", "Add a person", "Join with a code") — pass for that build only. Build 39 has **not** been run on a phone from this environment; the palette, wording and hub layout were verified from the app's own rendering in the screenshot harness. The full checklist and the ten-row paywall matrix are in `TEST_BUILD_39_ACCEPTANCE.md`; the paywall rows must be run on the **test_store** artifact, never the bypass one, and none of them may be reported as passing until a phone has done them. Trial, expiry and billing-issue rows depend on what the RevenueCat Test Store can simulate.
