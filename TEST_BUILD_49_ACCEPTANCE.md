...
4. **Deployed TEST backend**: f1f6b92 was deployed on 9 September 2026 at 23:37Z (run `run-20260909233730-629825`, image `sha256:9be0d3e4…f959bd`); its verify stopped at the twelfth script on the auth rate limiter (a test-setup limit, not a result) and is being completed with `verify --resume`. Until that reports passed, treat the backend as deployed but not fully verified. Before it, TEST was verified at **6a1115f** (the build-46 pin) on 9 September 2026, run directory `run-20260909042910-293693`, image `sha256:95fe66e1…a65cb`, 40/40 consent checks and ten regression scripts passed, scheduler OFF. Everything from builds 43 to 46 is therefore live on TEST. What is **not** yet on TEST is only the build-47 backend delta (below) plus the dedupe fix; until that rollout, the items marked "needs redeploy" run against the build-46 server.
