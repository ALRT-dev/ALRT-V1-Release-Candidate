...
1. **Installed build**: Profile footer, e.g. `ALRT 1.0.5 (47) · TEST build · <commit> · RevenueCat Test Store`. The build number and commit come from the footer, never from how a screen looks.
...
3. **App code**: the commit in the footer must equal the pin in the handover for build 47.
4. **Deployed TEST backend**: `https://api-test.safetyalrt.com/health` (or the `verify` stage's output) names the running revision. Until the build-47 pin is rolled out the server is still at **1230728**, and the items marked "needs redeploy" below run against the old server: check-in alert titles, leave outcomes, cross-circle SOS lists, private lock-screen previews, the test-notification action and token removal on sign-out all need the new server.
...
**Known defects and gaps not fixed in build 47 (listed so they are not mistaken for phone errors):** one public alert can arrive twice when it is both in a saved area and near a family member (`viewHazard` and `familyHazardProximity` paths); no background-message handler is registered (harmless while every push carries a notification block, but a data-only push would be dropped in the background); iOS entitlements use `aps-environment` production in the dev build (verify with the signing profile before an iOS test); no time-sensitive entitlement on iOS; family pushes are not gated by the user's category switches (by design); deleting the account does not clear the widget; the iOS widget target is not in the Xcode project.
...
Three separate columns; nothing in one column counts for another. "TEST server" stays "not deployed" until the build-47 pin is rolled out.

| Item | Automated (this environment) | TEST server | Phone |
|---|---|---|---|
| 1 SOS returns to Family | `family_sos_screen_navigation_test.dart` (5, through a real router on a 360×780 phone surface: leaves by itself after the server confirms, Done leaves once and the timer never navigates again, a failed send shows an error and stays, a lost answer is found on the server and sent once, a second hold in flight sends nothing extra); `family_sos_flow_test.dart` (9) | not deployed | item 1 |
| 2 No recipient screen for the sender | `family_sos_flow_test.dart`; `family_sos_authorization_test.dart` (mine across circles); receiver screen keeps sender wording while standing down and reports a failed stand-down | not deployed | item 2 |
...
| 5 Check-in strip on every alert | `family_safe_strip_test.dart` (9: five kinds, change circle, no circle, loading, no auto send); coverage table above | not deployed (alert title) | item 5 |
...
| 7 Notification settings alignment | `hazard_visibility_util_test.dart`, `verify_alert_filter_matrix` (56), `verify_push_notification_settings` (15) | not deployed | item 7; gaps need decisions |
| 8 Leave a circle | `family_leave_circle_test.dart` (4); backend `verify_family_leave_and_alert_link` §1 | not deployed | item 8 |
